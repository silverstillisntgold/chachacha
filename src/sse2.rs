...
impl Machine for Matrix {
    ...
    fn increment_djb(&mut self) {
        unsafe {
            self.state[0][3] = _mm_add_epi64(self.state[0][3], _mm_set_epi64x(0, DEPTH_U8.into()));
            self.state[1][3] = _mm_add_epi64(self.state[1][3], _mm_set_epi64x(0, DEPTH_U8.into()));
            self.state[2][3] = _mm_add_epi64(self.state[2][3], _mm_set_epi64x(0, DEPTH_U8.into()));
            self.state[3][3] = _mm_add_epi64(self.state[3][3], _mm_set_epi64x(0, DEPTH_U8.into()));
        }
    }
    ...
    fn increment_ietf(&mut self) {
        unsafe {
            self.state[0][3] =
                _mm_add_epi32(self.state[0][3], _mm_set_epi32(0, 0, 0, DEPTH_U8.into()));
            self.state[1][3] =
                _mm_add_epi32(self.state[1][3], _mm_set_epi32(0, 0, 0, DEPTH_U8.into()));
            self.state[2][3] =
                _mm_add_epi32(self.state[2][3], _mm_set_epi32(0, 0, 0, DEPTH_U8.into()));
            self.state[3][3] =
                _mm_add_epi32(self.state[3][3], _mm_set_epi32(0, 0, 0, DEPTH_U8.into()));
        }
    }
}
